...
object SubwaySurfersManager {
    ...
    private fun renderTicks(client: MinecraftClient, textRenderer: TextRenderer, matrixStack: MatrixStack): Float {
        val ticksSinceStart = String.format("%06d", ticks)
        val x: Float = client.window.scaledWidth - 2f - textRenderer.getWidth(ticksSinceStart)
        textRenderer.drawWithShadow(matrixStack, ticksSinceStart, x, 0f, 14737632)
        return x
    }
